...
func (f *Filestore) DeleteDir(path string, meta interface{}) (*types.Response, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	return f.config.Transport.DeleteDir(ctx, meta, f.config.Project, path, f.config.Token)
}
