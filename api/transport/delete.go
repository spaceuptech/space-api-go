...
func (t *Transport) Delete(ctx context.Context, meta *model.Meta, op string, find utils.M) (*model.Response, error) {

	// Make url for request
	url := t.generateDatabaseURL(meta, utils.Delete)

	// Fire the http request
	status, result, err := t.makeHTTPRequest(meta.Token, url, utils.M{"find": find, "op": op})
	if err != nil {
		return nil, err
	}
