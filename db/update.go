...
func initUpdate(db, col, op string, config *config.Config) *Update {
	meta := &model.Meta{Col: col, DbType: db, Project: config.Project, Token: config.Token, Operation: utils.Update}
	f := make(utils.M)
	u := make(utils.M)
...
func (u *Update) Apply(ctx context.Context) (*model.Response, error) {
